...
@register_model("pmsr")
class PMSRModel(TransformerModel):
    ...
    def hub_models(cls):
        return {
            "pmsr.base": None,
            "pmsr.large": None,
            "pmsr.large.mnli": None,
            "pmsr.large.cnn": None,
            "pmsr.large.xsum": None,
        }
    ...
    def build_model(cls, args, task):
        ...
        pmsr_base_architecture(args)
        if not hasattr(args, "max_positions"):
            args.max_positions = args.tokens_per_sample
        src_dict, tgt_dict = task.source_dictionary, task.target_dictionary
        if src_dict != tgt_dict:
            raise ValueError("PMSR requires a joined dictionary")
        ...
        encoder = PMSREncoder(args, src_dict, encoder_embed_tokens)
        decoder = PMSRDecoder(args, tgt_dict, decoder_embed_tokens)

        return PMSRModel(args, encoder, decoder)
...
class PMSRDecoder(TransformerDecoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.args = TransformerConfig.from_namespace(args)
        ...
        # modified by yinjie: remove the pgn % 07-30-2022
        ...
        self.decoder_layerdrop = self.args.decoder_layerdrop
        ...
        if not args.adaptive_input and self.args.quant_noise_pq > 0:
        ...
        self.embed_positions = (
            ...
            if not self.args.no_token_positional_embeddings
            else None
        )
    ...
    def extract_features_scriptable(
        ...
        return x, {"x": x, "attn": [attn], "inner_states": inner_states, "masked_encoder_out": encoder_out['masked_encoder_out'], "encoder_out": encoder_out["encoder_out"][0]}
...
class PMSREncoder(TransformerEncoder):
    """
    ...
    def contractive_forward(self, emb, target, return_all_hiddens=True):
        encoder_padding_mask = target.eq(self.padding_idx)
        has_pads = target.device.type == "xla" or encoder_padding_mask.any()
        x = emb
        # B x T x C -> T x B x C
        x = x.transpose(0, 1)
        ...
        src_lengths = target.ne(self.padding_idx).sum(dim=1, dtype=torch.int32).reshape(-1, 1).contiguous()
        return {
...
@register_model_architecture("pmsr", "pmsr_large")
def pmsr_large_architecture(args):
...
@register_model_architecture("pmsr", "pmsr_base")
def pmsr_base_architecture(args):
    ...
    pmsr_large_architecture(args)


@register_model_architecture("pmsr", "mpmsr_large")
def mpmsr_large_architecture(args):
    args.no_scale_embedding = getattr(args, "no_scale_embedding", False)
    pmsr_large_architecture(args)


@register_model_architecture("pmsr", "mpmsr_base")
def mpmsr_base_architecture(args):
    args.no_scale_embedding = getattr(args, "no_scale_embedding", False)
    pmsr_base_architecture(args)


@register_model_architecture("bart", "mpmsr_base_wmt20")
def mpmsr_base_wmt20_architecture(args):
    args.layernorm_embedding = getattr(args, "layernorm_embedding", False)
    mpmsr_base_architecture(args)


# class PMSRClassificationHead(nn.Module):
...
# class PMSREncoder(FairseqEncoder):
#     """
