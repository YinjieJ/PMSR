...
def collate(
    ...
    if samples[0].get("target", None) is not None:
        ...
        if samples[0].get("prev_output_tokens", None) is not None:
            ...
            prev_output_tokens = merge(
                ...
                pad_to_length=pad_to_length["masked_target"]
                if pad_to_length is not None
                else None,
            )
    else:
    ...
    if samples[0].get("label", None) is not None:
        batch["label"] = torch.LongTensor([s["label"] for s in samples])
...
class LanguagePairDataset(FairseqDataset):
    ...
    def __init__(
        ...
        self.masked_tgt = masked_tgt
        self.label = label
        self.src_sizes = np.array(src_sizes)
        self.tgt_sizes = np.array(tgt_sizes) if tgt_sizes is not None else None
        self.masked_src_sizes = np.array(masked_src_sizes)
        self.masked_tgt_sizes = np.array(masked_tgt_sizes)
        ...
        if num_buckets > 0:
            ...
            self.masked_tgt = BucketPadLengthDataset(
                self.masked_tgt,
                sizes=self.masked_tgt_sizes,
                num_buckets=num_buckets,
                pad_idx=self.src_dict.pad(),
                left_pad=self.left_pad_target,
            )
    ...
    def __getitem__(self, index):
        ...
        masked_tgt_item = self.masked_tgt[index]
        label = None
        ...
        if self.append_eos_to_target:
            ...
            if self.masked_tgt and self.masked_tgt[index][-1] != eos:
                tgt_item = torch.cat(
                    [self.masked_tgt[index], torch.LongTensor([eos])])
