...
def spanmask(mask, num_mask, sz):
    lower, upper = 1, 10
    p = 0.2
    lens = list(range(lower, upper + 1))
    len_distrib = [p * (1-p)**(i - lower) for i in range(lower, upper + 1)]
    len_distrib = [x / (sum(len_distrib)) for x in len_distrib]
    masked_num = num_mask
    mask_type = np.random.rand(sz)
    if masked_num > 0:
        span = []
        l_masks = 0
        while True:
            start = np.random.choice(sz)
            if mask[start] == True:
                continue
            span_len = np.random.choice(lens, p=len_distrib)
            span_len = min(masked_num - l_masks, span_len)
            end = min(start + span_len, sz)
            mask[start:end] = True
            span.append([start, end])
            l_masks = mask.sum()
            if l_masks >= masked_num:
                break
        span = merge_intervals(span)
        for [start, end] in span:
            mask_type[start:end] = mask_type[start]
    return mask, mask_type

class SpanMaskTokensDataset(BaseWrapperDataset):
    """
    ...
    def __getitem_cached__(self, seed: int, epoch: int, index: int):
        with data_utils.numpy_seed(self.seed, self.epoch, index):
            ...
            mask, mask_type = spanmask(mask, num_mask, sz)
            ...
            if rand_or_unmask_prob > 0.0:
                rand_or_unmask = mask & (mask_type < rand_or_unmask_prob)
