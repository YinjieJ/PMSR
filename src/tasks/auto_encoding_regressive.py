...
from ..data.span_mask_dataset import SpanMaskTokensDataset
...
def load_langpair_dataset(
    ...
    # mask source dataset.
    src_dataset, masked_src_dataset = SpanMaskTokensDataset.apply_mask(
        src_dataset,
        src_dict,
    ...
    # mask target dataset.
    masked_tgt_dataset,_ = SpanMaskTokensDataset.apply_mask(
        tgt_dataset,
        tgt_dict,
        pad_idx=src_dict.pad(),
        mask_idx=mask_idx,
...
class AutoEncodeingRegressiveTask(LegacyFairseqTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            "--constractive", default=True,  action='store_true', help="is using constractive loss"
        )
        # fmt: on
        # parser.add_argument(
        #     "--sent-loss",
    ...
    def __init__(self, args, src_dict, tgt_dict, label_dictionary=None):
        super().__init__(args)
    ...
    def setup_task(cls, args, **kwargs):
        ...
        label_dict = None
        if args.constractive:
            label_dict = cls.load_dictionary(
            os.path.join(paths[0], "label", "dict.txt")
        )
